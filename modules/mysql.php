...
if (!(function_exists("mysql_connect"))) {
	function mysql_connect($mysql_servername = null, $mysql_username = null, $mysql_db_name = null, $mysql_password = null, $connection_id = 0, $mysql_charset = "utf8mb4")
	{
		...
		$prev = mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT);
...
{
	if (
		!isset($GLOBALS["mysql_connections"]) ||
		!isset($GLOBALS["mysql_connections"][$connection_id]) ||
		!($GLOBALS["mysql_connections"][$connection_id] instanceof mysqli)
	) {
		$GLOBALS["mysql_connections"][$connection_id] = mysql_connect();
	}
...
function mysql_prepared_execute($query, $params = [], $conn = false)
{
	...
	if (!empty($params)) {
		...
		foreach ($params as &$param) {
			if (is_bool($param)) {
				...
				$types .= 's'; /* mysqli sends NULL correctly */
			} else {
				$types .= 's';
			}
			$values[] = $param;
	...
	$stmt->execute();
	if ($stmt->field_count > 0) {  // result set present
...
function mysql_update($table, $data, $where, $limit = null, $conn = false)
{
	...
	$GLOBALS["last_db_query"][] = mysql_return_final_query($query, $params);
