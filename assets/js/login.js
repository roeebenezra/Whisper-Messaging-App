  ...
  document.addEventListener("click", function (e) {
    ...
    if (el.id === "btn_send_otp") {
      ...
      send("request_otp", { username: username, website: website }, function (json) {
        ...
        // show the OTP container and input
        var otpGroup = document.getElementById("login_otp_group");
        if (otpGroup) otpGroup.style.display = "";           // <-- show the wrapper

        var otpInput = document.getElementById("login_otp");
        if (otpInput) {
          otpInput.style.display = "";
          otpInput.focus();                                 
        }

        // show the verify button & disable send
